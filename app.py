...
def cornish_fisher_var(x, c=0.95):
    ...
    s = float(stats.skew(r, bias=False)); k = float(stats.kurtosis(r, fisher=True, bias=False))
...
def ann_vol(x):
    r = _arr(x); return float(r.std(ddof=1)*np.sqrt(TRADING_DAYS)) if r.size>1 else np.nan
...
def section_card(title, children, icon=""):
    return dbc.Card([
        dbc.CardHeader(html.H5(f"{icon} {title}", className="mb-0", style={"color":BRAND})),
        dbc.CardBody(children)
    ], className="mb-3 shadow-sm")

def info_alert(items):
    return dbc.Alert(
        [html.Div([html.Strong(f"{k}: "), v]) for k, v in items],
        color="light", className="mb-3", style={"fontSize":"0.85rem"})

app = Dash(__name__,
           external_stylesheets=[dbc.themes.FLATLY],
           title="Crypto Risk Dashboard — UNIGE",
           suppress_callback_exceptions=True)
server = app.server  # required for gunicorn

header = dbc.Card(dbc.CardBody([
    html.Div("Université de Genève · Geneva School of Economics and Management",
             style={"fontSize":"0.8rem","color":"#bbd"}),
    html.H2("Crypto Market & Liquidity Risk Dashboard",
            style={"color":"white","marginTop":"4px","marginBottom":"4px"}),
    html.Div(["Quantitative Risk Management (Spring 26) · ", html.B("Group 3")],
             style={"fontSize":"0.9rem","color":"#dde6f5"}),
...
def portfolio_inputs():
    return [dbc.Row([
        ...
        dbc.Col(dbc.Input(id={"type":"hold","index":a}, type="number",
                          min=0, step=10000, value=float(DEFAULT_HOLDINGS[a]), size="sm"), width=8),
...
sidebar = dbc.Card(dbc.CardBody([
    ...
    html.Div(portfolio_inputs(), style={"marginTop":"8px"}),
    html.Hr(),
    html.Label("VaR / ES confidence (α)", style={"fontWeight":600,"fontSize":"0.85rem"}),
    dcc.Slider(id="alpha-slider", min=0.90, max=0.99, value=DEFAULT_ALPHA, step=0.01,
               marks={0.90:"90%", 0.95:"95%", 0.99:"99%"}),
    html.Br(),
    html.Label("Liquidity coefficient (λ, BDSS)", style={"fontWeight":600,"fontSize":"0.85rem"}),
    dcc.Slider(id="lambda-slider", min=0.0, max=2.0, value=DEFAULT_LAMBDA, step=0.05,
               marks={0:"0", 0.5:"0.5", 1:"1.0", 2:"2.0"}),
    html.Br(),
    html.Label("Stress scenario", style={"fontWeight":600,"fontSize":"0.85rem"}),
    dcc.Dropdown(id="scenario-dd",
                 options=[{"label":k,"value":k} for k in SCENARIOS.keys()],
                 value=list(SCENARIOS.keys())[0], clearable=False,
                 style={"fontSize":"0.85rem"}),
    html.Hr(),
    html.Div(id="sidebar-summary", style={"fontSize":"0.78rem","color":"#666"}),
...
tabs = dcc.Tabs(id="tabs", value="tab-market", children=[
    ...
    dcc.Tab(label="📉 VaR & ES",       value="tab-var"),
    dcc.Tab(label="💧 Liquidity",      value="tab-liq"),
    dcc.Tab(label="🌪 Crash Scenarios",value="tab-scen"),
...
def render_market(holdings, alpha):
    ...
    fig_vol = px.line(df, x="date", y="rolling_vol_30", color="asset",
                      title="30d rolling volatility (ann. √365 — crypto 365d/yr)", height=320)
    fig_vol.update_layout(template="plotly_white", yaxis_tickformat=".0%", legend_title="")
    fig_dd = px.line(df, x="date", y="drawdown", color="asset",
                     title="Drawdown from peak", height=320)
    fig_dd.update_layout(template="plotly_white", yaxis_tickformat=".0%", legend_title="")
    ...
    return html.Div([
        ...
        dbc.Row([
            ...
            dbc.Col(kpi_card("7-day return",   pctf(ret_7d,2),"Trailing")),
            dbc.Col(kpi_card("Max drawdown",   pctf(mdd,1),   "Equal-weight")),
            dbc.Col(kpi_card("Sentiment",      senti,         "7d return", sc)),
        ], className="g-2 mb-3"),
        dbc.Row([dbc.Col(dcc.Graph(figure=fig_norm),md=6),
                 dbc.Col(dcc.Graph(figure=fig_vol), md=6)]),
        dbc.Row([dbc.Col(dcc.Graph(figure=fig_dd),  md=6),
...
def render_var(holdings, alpha):
    df = FEATURES.copy()
    port_ret = portfolio_returns_from(df, holdings).dropna()
    if port_ret.empty:
        return dbc.Alert("No return data.", color="warning")
    ...
    fig_h = go.Figure()
    fig_h.add_trace(go.Histogram(x=port_ret*100, nbinsx=60, marker_color="#9aa9c2"))
    v0 = methods["Historical Simulation"]
    if not np.isnan(v0):
        fig_h.add_vline(x=-v0*100, line_color="red",    line_dash="dash",
                        annotation_text=f"VaR {int(alpha*100)}%")
    if not np.isnan(es):
        fig_h.add_vline(x=-es*100, line_color="darkred", line_dash="dot",
                        annotation_text=f"ES {int(alpha*100)}%")
    fig_h.update_layout(title="Return distribution", template="plotly_white",
    ...
    if roll_df.empty:
        fig_roll = go.Figure().update_layout(title="Rolling VaR & ES (need 250+ days)")
    else:
        fig_roll = px.line(roll_df, x="date", y=["Rolling VaR","Rolling ES"],
                           title=f"Rolling VaR & ES (250d, {int(alpha*100)}%)")
        fig_roll.update_layout(template="plotly_white", height=320,
    ...
    fig_cvar = (px.bar(cvar_df, x="Asset", y="% of Portfolio VaR", color="Asset",
                       title="Component VaR (Euler decomposition)")
                if cvar_df is not None
                else go.Figure().update_layout(title="Component VaR (insufficient data)"))
    ...
    return html.Div([
        dbc.Row([
            dbc.Col(section_card(f"VaR comparison @ {int(alpha*100)}%", [
                dash_table.DataTable(
                    columns=[{"name":c,"id":c} for c in rows[0].keys()],
                    data=rows,
                    style_cell={"fontFamily":"system-ui","padding":"6px","fontSize":"0.82rem"},
                    style_header={"backgroundColor":"#f4f6fa","fontWeight":600})
            ], icon="📊"), md=6),
            dbc.Col(section_card("Distribution", [dcc.Graph(figure=fig_h)], icon="📉"), md=6),
        ]),
        section_card("Kupiec POF backtest",
                     [html.Ul([html.Li(x) for x in kp])], icon="🧪"),
        section_card(f"Per-asset @ α={int(alpha*100)}%", [
            dash_table.DataTable(
                columns=[{"name":c,"id":c} for c in per[0].keys()] if per else [],
                data=per,
                style_cell={"fontFamily":"system-ui","padding":"6px","fontSize":"0.82rem"},
                style_header={"backgroundColor":"#f4f6fa","fontWeight":600})
        ], icon="🪙"),
        dbc.Row([
            dbc.Col(section_card("Component VaR",   [dcc.Graph(figure=fig_cvar)], icon="🧮"), md=6),
            dbc.Col(section_card("Rolling VaR & ES",[dcc.Graph(figure=fig_roll)], icon="⏱"),  md=6),
        ]),
    ])
...
def render_liq(holdings, alpha, lam):
    df = FEATURES.copy()
    latest = df.sort_values("date").groupby("asset").tail(1).set_index("asset")
    AUM = sum(holdings.values()) or 1.0
    rows = []; total_lc = 0.0
    for a in ASSETS:
        ...
        sd  = df[df["asset"]==a]["return"].std(ddof=1)
    ...
    port_ret = portfolio_returns_from(df, holdings).dropna()
    std_var_pct = hist_var(port_ret, alpha)
    ...
    for L in lam_grid:
        tot = sum(liquidity_cost(
                    ...
                    df[df["asset"]==a]["return"].std(ddof=1), L)
                  for a in ASSETS if a in latest.index)
        sens.append({"lambda":L, "Liquidity cost":tot,
                     "L-VaR":(std_var_usd or 0)+tot})
    sens_df = pd.DataFrame(sens)
    fig_lam = go.Figure()
    fig_lam.add_trace(go.Scatter(x=sens_df["lambda"], y=sens_df["Liquidity cost"],
                                  name="Liquidity cost", mode="lines+markers"))
    fig_lam.add_trace(go.Scatter(x=sens_df["lambda"], y=sens_df["L-VaR"],
                                  name="L-VaR", mode="lines+markers"))
    if std_var_usd and not np.isnan(std_var_usd):
        fig_lam.add_hline(y=std_var_usd, line_dash="dash", line_color="gray",
    ...
    return html.Div([
        ...
        section_card(f"Per-asset liquidity (λ={lam:.2f}, α={int(alpha*100)}%)", [
            dash_table.DataTable(
                ...
                data=rows,
                style_cell={"fontFamily":"system-ui","padding":"6px","fontSize":"0.78rem"},
                style_header={"backgroundColor":"#f4f6fa","fontWeight":600}),
            html.Div([html.B("Total LC: "), money(total_lc), "   ",
                      html.B("Std VaR: "), money(std_var_usd), "   ",
                      html.B("L-VaR: "),   money((std_var_usd or 0)+total_lc)],
                     style={"marginTop":"10px","color":BRAND,"fontSize":"0.9rem"})
        ], icon="💧"),
...
def render_scen(holdings, alpha, lam, scenario):
    ...
    latest = df.sort_values("date").groupby("asset").tail(1).set_index("asset")
    AUM = sum(holdings.values()) or 1.0
    rows = []; total_mtm = 0.0; total_stress_lc = 0.0
    for a in ASSETS:
        if a not in latest.index: continue
        h     = float(holdings.get(a, 0))
        ...
        rows.append({"Asset":a, "Position":money(h),
    ...
    fig_w.update_layout(
        title=f"{scenario} — loss decomposition",
    ...
    base_lc  = sum(liquidity_cost(
                    holdings.get(a,0),
                    float(latest.loc[a,"adv_30_usd"]) if a in latest.index else 0,
                    df[df["asset"]==a]["return"].std(ddof=1), lam)
                   for a in ASSETS if a in latest.index)
    fig_cmp = go.Figure([
    ...
    return html.Div([
        dbc.Alert([html.H5(scenario, style={"color":BRAND}),
                   html.Div(sc["description"])], color="warning"),
        dbc.Row([
            dbc.Col(kpi_card("MTM P&L",         money(total_mtm),
                             "Shock leg",        "#c0392b")),
            dbc.Col(kpi_card("Stressed LC",      money(total_stress_lc),
                             "Fire-sale leg",    "#e67e22")),
            dbc.Col(kpi_card("Total loss",       money(-total_loss),
                             f"{total_loss/AUM*100:.1f}% AUM","#8e44ad")),
        ], className="g-2 mb-3"),
        section_card("Waterfall",   [dcc.Graph(figure=fig_w)],   icon="🌊"),
        section_card("Per-asset",   [
            dash_table.DataTable(
                columns=[{"name":c,"id":c} for c in rows[0].keys()] if rows else [],
                data=rows,
                style_cell={"fontFamily":"system-ui","padding":"6px","fontSize":"0.78rem"},
...
@app.callback(
    ...
    Input({"type":"hold","index":ALL}, "value"),
    State({"type":"hold","index":ALL}, "id"),
    prevent_initial_call=True,
)
def update_all(tab, alpha, lam, scenario, vals, ids):
    try:
        holdings = {ids[i]["index"]: float(vals[i] or 0) for i in range(len(ids))}
        AUM = sum(holdings.values())
        if AUM <= 0:
            return (dbc.Alert("⚠ Set at least one holding > 0.", color="warning"),
                    [html.Div("AUM: $0")],
                    html.Div("Set positive holdings."))
        ...
        lc_total    = sum(
            liquidity_cost(holdings.get(a,0),
                           float(latest.loc[a,"adv_30_usd"]) if a in latest.index else 0,
        ...
        kpi = dbc.Row([
            dbc.Col(kpi_card("Portfolio AUM",               money(AUM),
                             f"{len(ASSETS)} assets")),
            dbc.Col(kpi_card(f"Std VaR {int(alpha*100)}%",  money(std_var_usd),
                             pctf(std_var_pct), "#c0392b")),
            dbc.Col(kpi_card(f"Liq. cost (λ={lam:.2f})",   money(lc_total),
                             f"{lc_total/AUM*100:.4f}% AUM","#e67e22")),
            dbc.Col(kpi_card(f"L-VaR {int(alpha*100)}%",    money(lvar),
                             pctf(lvar/AUM) if not np.isnan(lvar) else "—","#8e44ad")),
            dbc.Col(kpi_card("Ann. vol (√365)",              pctf(a_vol,1),"Crypto 365d/yr")),
        ], className="g-2")
        ...
        return (dbc.Alert(html.Pre(err, style={"whiteSpace":"pre-wrap"}), color="danger"),
                "", html.Div())
